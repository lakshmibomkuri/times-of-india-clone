...
const tabContent: Record<string, TabItem[]> = {
  ...
  "Recipes Videos": recipeVideos.map((v) => ({ image: v.image, title: v.title, duration: v.duration })),
};
  ...
  return (
    <section className="py-4 border-b border-dashed border-gray-300">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
            ...
            <div className="md:col-span-7">
              <div className="relative rounded-lg overflow-hidden">
                ...
                />
                <div className="absolute bottom-0 left-0 right-0 p-3 bg-gradient-to-t from-black/70 to-transparent">
                  <p className="text-white text-[13px] font-semibold leading-snug">{mainRecipe.title}</p>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            {featuredRecipes.slice(0, 4).map((recipe, idx) => (
              <div key={idx} className="cursor-pointer">
                ...
                <p className="text-[11px] text-gray-800 mt-1 leading-tight hover:text-[#e53935]">{recipe.title}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="md:col-span-5">
          {/* Tabs */}
          <div className="flex gap-4 mb-4 border-b border-gray-200 overflow-x-auto no-scrollbar">
            {tabs.map((tab) => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
          ...
          <div className="space-y-3">
            {(tabContent[activeTab] ?? []).map((item, idx) => (
              <div key={`${activeTab}-${idx}`} className="flex items-center gap-3 cursor-pointer">
                <div className="relative">
                  <Image src={item.image} alt={item.title} width={thumbW} height={thumbH} className={thumbClass} />
                  {item.duration && (
                    <>
                      <div className="absolute inset-0 flex items-center justify-center">
                        <div className="w-7 h-7 bg-[#e53935]/90 rounded-full flex items-center justify-center">
                ...
                <p className="text-[13px] text-gray-900 leading-snug hover:text-[#e53935]">{item.title}</p>
              </div>
            ))}
          </div>
        </div>

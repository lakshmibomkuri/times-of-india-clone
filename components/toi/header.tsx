  ...
  return (
    <>
      <header className="bg-white sticky top-0 z-50" style={{ fontFamily: 'Arial, Helvetica, sans-serif' }}>
          ...
          <div className="max-w-[1200px] mx-auto px-1">
            <div className="flex items-center justify-between h-[32px]">
              ...
              <div className="flex items-center gap-3 text-[11px] text-gray-600">
                ...
                <span>Tue, Jan 20, 2026 | Updated 06.31PM IST</span>
                <span className="text-gray-400">|</span>
              ...
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-2">
                  <Link href="#" className="w-5 h-5 flex items-center justify-center rounded-full border border-gray-300 hover:border-gray-400">
                    <svg className="w-2.5 h-2.5 text-[#1877f2]" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" /></svg>
                  </Link>
                  <Link href="#" className="w-5 h-5 flex items-center justify-center rounded-full border border-gray-300 hover:border-gray-400">
                  ...
                  <Link href="#" className="w-5 h-5 flex items-center justify-center rounded-full bg-[#ff0000] text-white">
        ...
        <div className="bg-white py-2">
          <div className="max-w-[1200px] mx-auto px-3 text-center">
            <Link href="/" className="inline-block">
              <h1 className="text-[32px] font-serif text-black tracking-tight" style={{ fontFamily: 'Times New Roman, Georgia, serif' }}>
                THE TIMES OF INDIA
              </h1>
            </Link>
          </div>
        ...
        <nav className="border-b-3 border-black bg-gray-100 relative m-0">
          <div className="max-w-[1200px] mx-auto">
            <div className="flex justify-between h-[40px]">
              ...
              <ul className="flex items-center gap-2 overflow-x-auto whitespace-nowrap scrollbar-hide lg:overflow-visible">

                <li>
                  <Link
                    href="#"
                    className="flex items-center px-2 py-1.5 text-[13px] text-black"
                  >
                    TOI
                    <span className="ml-0.5 text-[14px] font-bold text-[#e53935]">+</span>
                  </Link>
                </li>
                {navItems.map((item) => (
                  <li
                  ...
                  >
                    <Link
                      href={item.route ? item.route : "#"} // <-- use the route here
                      className={`flex items-center text-[13px] hover:text-[#e53935] transition-colors whitespace-nowrap ${activeMenu === item.label ? 'text-[#e53935]' : 'text-gray-800'}`}
                    >
                      {item.label}
                      {item.isNew && (
                        <span className="text-[9px] text-[#e53935] font-medium ml-0.5 -mt-3">New</span>
                      )}
                    </Link>
                  </li>
                ))}
                ...
                {activeMenu === "TOI Games" ? (
                  <div className="flex gap-6">
                    {toiGamesMenu.map((game) => (
                      <Link key={game} href="#" className="text-[13px] text-gray-700 hover:text-[#e53935] py-1">
                        {game}
                      </Link>
                    ))}
                  </div>
                  ...
                  <div className="flex gap-8">
                    {menuData[activeMenu].columns?.map((col, idx) => (
                      <div key={idx} className="min-w-[150px]">
                        {col.title && <h4 className="text-[11px] font-bold text-gray-500 mb-2 uppercase tracking-wide">{col.title}</h4>}
                        <div className={`${col.items.length > 15 ? 'grid grid-cols-3 gap-x-4' : ''}`}>
                          {col.items.map((subItem) => (
                            <Link key={subItem} href="#" className="block py-1 text-[13px] text-gray-700 hover:text-[#e53935]">
                              {subItem}
                            </Link>
                          ))}
                        </div>
                  ...
                  <div className="flex gap-6">
                    {menuData[activeMenu].items?.map((subItem) => (
                      <Link key={subItem} href="#" className="text-[13px] text-gray-700 hover:text-[#e53935] py-1">
                        {subItem}
                      </Link>
                    ))}
                  </div>
            ...
            <div className="max-w-[1200px] mx-auto px-3 py-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6">
                {Object.entries(fullMenuData).map(([category, data]) => (
                  <div key={category}>
                    <h4 className="text-[12px] font-bold text-gray-800 mb-2 flex items-center">
                      {category}
                    ...
                    <ul>
                      {data.items.map((item) => (
                        <li key={item}>
                          <Link href="#" className="block py-0.5 text-[11px] text-gray-600 hover:text-[#e53935]">
                            {item}
                          </Link>
                        </li>
                      ))}
        ...
        {/* Secondary Navigation - In The News */}
        <div className="border-b border-gray-200 bg-white overflow-hidden p-1 overflow-auto">
          <div className="max-w-[1200px] mx-auto">
            <div className="flex items-center gap-4 py-1.5 text-[12px]">
              <span className="text-[#e53935] flex-shrink-0 font-medium">In The News</span>
              <div className="flex items-center gap-4 overflow-x-auto scrollbar-hide">
                {inTheNews.map((topic, i) => (
                  <Link key={i} href="#" className="text-gray-700 hover:text-[#e53935] flex-shrink-0 whitespace-nowrap flex items-center">
                    {topic.text}
                    {topic.isNew && <span className="text-[9px] text-[#e53935] font-medium ml-0.5 -mt-2">New</span>}
                  </Link>
                ))}
              </div>
            </div>
        ...
        {searchOpen && (
          ...
          >
            <div
              className="bg-white w-[900px] mt-[90px] mx-auto border border-gray-200"
            ...
            >
              ...
              <div className="flex items-center gap-2 px-4 py-3 border-b border-gray-200">
                <input
                  ...
                  className="flex-1 h-[38px] px-3 border border-gray-300 text-[14px] focus:outline-none focus:border-[#e53935]"
                />

                <button className="h-[38px] px-6 bg-[#e53935] text-white text-[13px] font-semibold">
                  SEARCH
                </button>

                <button
                  onClick={() => setSearchOpen(false)}
                  className="ml-2 text-gray-500 hover:text-black text-[20px]"
                >
                  ×
                </button>
              </div>
              ...
              <div className="px-4 py-3">
                <div className="flex items-center gap-2 text-[13px] font-semibold text-gray-700 mb-2">
                  <span className="inline-block w-4 h-4 rounded-full border border-gray-400 text-[10px] flex items-center justify-center">
                    ↗
                  </span>
                  TRENDING
                </div>
